...
class TrafficEnvironment:
    ...
    def traffic_demand(self, vi, vj, one_path: Path):
        ...
        single_path = self.w_ * self.travel_cost(one_path)
        ...
        for row in all_Paths:
            ...
            sum_all_Paths += self.w_ * self.travel_cost(p)
    ...
    def step(self,
        ...
        for e in self.edges:
            end_point = e.end
            rewards += int((self.state_matrix[e.id][end_point] * self.period_time) / (self.free_flow_speed * (1 + self.A * (sum(self.state_matrix[e.id]) / self.capacity_of_road) ** self.B)))
